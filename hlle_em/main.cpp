...
double Tcutphot, Tcphot, Tcutdilept;
...
int main(int argc, char **argv) {
  ...
  PhotonsAMYCUDA *phot = new PhotonsAMYCUDA(tau0, taus, Tcphot);
  phot->setMode(1); // Upper limit
  for(int i=0;i<pTphot.size();++i)
	  phot->addPtY(pTphot[i], Yphot[i]);
  phot->initCUDAArrays(f);
  
  PhotonsMixCUDA *phot2 = new PhotonsMixCUDA(tau0, taus, Tcutphot, Tcphot);
  phot2->setMode(1); // Upper limit
  for(int i=0;i<pTphot.size();++i)
