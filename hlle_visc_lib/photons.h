...
class PhotonsHadrons : public Photons
{
	double tau0, taus;                // Undersaturated QGP
	std::vector<double> xphi, wphi;   // Legendre quadrature to integrate over phi
	std::vector<double> xeta, weta;   // Laguerre quadrature to integrate over eta (for boost-invariant case)
	int fMode;
public:
	PhotonsHadrons(double tau0_=0.1, double taus_=0., double Tcut_ = 0.155) : Photons(Tcut_), tau0(tau0_), taus(taus_), fMode(0) { init(); }
	PhotonsHadrons(const std::vector<double> & ptin, const std::vector<double> & yin, double tau0_=0.1, double taus_=0., double Tcut_ = 0.155);
	void init();
	void setMode(int mode_ = 0) { fMode = mode_; }
	virtual ~PhotonsHadrons();// { }
	virtual void addCell(double tau, double dtau, Fluid *f, EoS *eos, Cell *c) { }
	virtual void addCellBI(double tau, double dtau, Fluid *f, EoS *eos, Cell *c);
	virtual void addCellBISymm(double tau, double dtau, Fluid *f, EoS *eos, Cell *c);
...
class PhotonsMix : public Photons
{
	double coef;
	double coefBI;
	double A1, A2;
	...
	int fMode;
public:
	PhotonsMix(double tau0_=0.1, double taus_=0., double Tcut_ = 0.155, double Tc_ = 0.155) : Photons(Tcut_), TcAMY(Tc_), tau0(tau0_), taus(taus_), fMode(0) { init(); }
	PhotonsMix(const std::vector<double> & ptin, const std::vector<double> & yin, double tau0_=0.1, double taus_=0., double Tcut_ = 0.155, double Tc_ = 0.155);
	void init();
	void setMode(int mode_ = 0) { fMode = mode_; }
	virtual ~PhotonsMix() { }
	virtual void addCell(double tau, double dtau, Fluid *f, EoS *eos, Cell *c);
