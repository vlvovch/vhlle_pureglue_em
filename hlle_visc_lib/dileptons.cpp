...
void DileptonsQGP::addCell(double tau, double dtau, Fluid *f, EoS *eos, Cell *c) {
	...
	for(int ic = 0; ic < Ms.size(); ++ic) {
		...
		for(int iqt = 0; iqt < xeta.size(); ++iqt) {
			double tqt  = xeta[iqt];
			double tsum = lambda * lambda * t * t * tqt * BesselI0(gamma * vT * tqt) * exp(-gamma * sqrt(Mtil*Mtil+tqt*tqt) * (cosh(Y) - tanh(vz) * sinh(Y)));
			sum += weta[iqt] * tsum;
		}
...
void DileptonsQGP::addCellBI(double tau, double dtau, Fluid *f, EoS *eos, Cell *c) {
	...
	for(int ic = 0; ic < Ms.size(); ++ic) {
		...
		/*if (0) {
			for(int iqt = 0; iqt < xeta.size(); ++iqt) {
				double tqt  = xeta[iqt];
				double tsum = lambda * lambda * t * t * tqt * ModifiedBesselI0(gamma * vT * tqt) * ModifiedBesselK0(gamma * sqrt(Mtil*Mtil+tqt*tqt));
				sum += weta[iqt] * tsum;
			}
		}
		else*/ 
		{
			sum = lambda * lambda * t * M * BesselK1(M / t);
			if (vT>1.e-6) sumv2 = lambda * lambda * (vx*vx - vy*vy) / (vx*vx + vy*vy) * (t * M * BesselK1(M / t) - 2. * t * t / (gamma*gamma-1.) * (BesselK0(M / t)-BesselK0(gamma * M / t) ) ) ;
			else sumv2 = lambda * lambda * (vx*vx - vy*vy) / 4. * M * M * BesselK(2, M / t);
		}
