...
int main(int argc, char **argv) {
  ...
  if(icModel==1){ // optical Glauber
   ...
   ic->setIC(f, eos, tau0);
   delete ic;
  }else if(icModel==6){ // hard Sphere profile
   ICHardSpheres *ic = new ICHardSpheres(epsilon0, 0., tau0, 0., 1., 0.6);
   ic->setIC(f, eos);
   delete ic;
  }else {
  ...
  PhotonsMixCUDA *phot = new PhotonsMixCUDA(tau0, taus, Tcutphot, 0.155);
  phot->setMode(0);
  for(int i=0;i<pTphot.size();++i)
	  phot->addPtY(pTphot[i], Yphot[i]);

  phot->initCUDAArrays(f);
  
  PhotonsAMYCUDA *phot2 = new PhotonsAMYCUDA(tau0, taus, Tcutphot);
  phot2->setMode(1);
  for(int i=0;i<pTphot.size();++i)
	  phot2->addPtY(pTphot[i], Yphot[i]);

  phot2->initCUDAArrays(f);
  ...
  for (int istep = 0; istep < maxstep; istep++) {
    ...
    for (int j = 0; j < nSubSteps; j++) {
      ...
      if (pTphot.size() > 0) {
        phot->addCellsBI(h->getTau(), dtau, f, f->getEoS());
        phot2->addCellsBI(h->getTau(), dtau, f, f->getEoS());
      }
