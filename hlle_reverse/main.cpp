...
double epsilon0, Rgt, Rgz, impactPar, s0ScaleFactor, alphaGlauber;
...
int main(int argc, char **argv) {
  ...
  if(icModel==1){ // optical Glauber
   ICGlauber *ic = new ICGlauber(epsilon0, impactPar, tau0, 208, 6.62, 0.546, 6.4);
   //ic->setIC(f, eos);
  ...
  /*Photons *phot = new PhotonsQGP(tau0, taus);
  //phot->addPtY(0.1, 0.0);
  phot->addPtY(0.5, 0.0);
  phot->addPtY(1.0, 0.0);
  phot->addPtY(1.5, 0.0);
  ...
  // Reverse
  int substepsadd = 5;
  for (int istep = 0; istep < maxstep / substepsadd; istep++) {
    // decrease timestep automatically, but use fixed dtau for output
    int nSubSteps = 1;
    //while (dtau / nSubSteps > 1.0 * (tau0 + dtau * istep) * (etamax - etamin) / (nz - 1))
    //  nSubSteps *= 2;  // 0.02 in "old" coordinates
    h->setDtau(-dtau / nSubSteps * (double)(substepsadd));
    //cout<<"dtau = "<<dtau / nSubSteps<<endl;
    for (int j = 0; j < nSubSteps; j++) {
      h->performStep();
	  //f->processPhotons(h->getTau(), dtau, phot, photmode);
